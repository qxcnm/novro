...
func (_q *APIKeyQuery) QueryAPIUsages() *APIUsageQuery {
...
func (_q *APIKeyQuery) WithAPIUsages(opts ...func(*APIUsageQuery)) *APIKeyQuery {
	query := (&APIUsageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) loadAPIUsages(ctx context.Context, query *APIUsageQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *APIUsage)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apiusage.FieldAPIKeyID)
	}
