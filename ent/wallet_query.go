...
func (_q *WalletQuery) QueryEntries() *WalletEntryQuery {
...
func (_q *WalletQuery) WithEntries(opts ...func(*WalletEntryQuery)) *WalletQuery {
	query := (&WalletEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *WalletQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Wallet, error) {
	...
	if query := _q.withEntries; query != nil {
		if err := _q.loadEntries(ctx, query, nodes,
			func(n *Wallet) { n.Edges.Entries = []*WalletEntry{} },
			func(n *Wallet, e *WalletEntry) { n.Edges.Entries = append(n.Edges.Entries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *WalletQuery) loadEntries(ctx context.Context, query *WalletEntryQuery, nodes []*Wallet, init func(*Wallet), assign func(*Wallet, *WalletEntry)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(walletentry.FieldWalletID)
	}
	query.Where(predicate.WalletEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(wallet.EntriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
