...
func (_q *ProviderQuery) QueryModelRoutes() *ModelRouteQuery {
...
func (_q *ProviderQuery) WithModelRoutes(opts ...func(*ModelRouteQuery)) *ProviderQuery {
	query := (&ModelRouteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProviderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Provider, error) {
	...
	if query := _q.withModelRoutes; query != nil {
		if err := _q.loadModelRoutes(ctx, query, nodes,
			...
			func(n *Provider, e *ModelRoute) { n.Edges.ModelRoutes = append(n.Edges.ModelRoutes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProviderQuery) loadModelRoutes(ctx context.Context, query *ModelRouteQuery, nodes []*Provider, init func(*Provider), assign func(*Provider, *ModelRoute)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(modelroute.FieldProviderID)
	}
	query.Where(predicate.ModelRoute(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(provider.ModelRoutesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
