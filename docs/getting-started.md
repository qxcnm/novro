...
管理员初始化完成前，注册和 OIDC 自动注册都会被拒绝。本地注册需要用户名、邮箱、密码和邮箱验证码，用户名同时作为初始显示名称；
...
取得连接前的明确连接失败会有限重试或切换；明确的非 `2xx` 响应会在当前渠道重试两次，三次请求均失败后切换下一候选。请求结果不明、成功响应体不完整或流已经开始后不会自动重放。
