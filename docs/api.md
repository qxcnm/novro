...
| `GET` | `/api/admin/billing-groups` | 管理员 | 查看分组、基础/当前倍率、定时优惠、授权用户、API Key 数和模型路由数 |
| `POST` | `/api/admin/billing-groups` | 管理员 | 创建计费分组、倍率、可选定时优惠和隐藏组授权用户 |
| `PATCH` | `/api/admin/billing-groups/{id}` | 管理员 | 修改分组资料或定时优惠；`clear_discount=true` 清除优惠 |
...
同一上游模型可以与多个计费分组各建立一条绑定。请求和管理端始终传递 `billing_group_id`；`multiplier_bps` 仅用于计费，不作为路由标识。定时优惠使用 `{name, multiplier_bps, starts_at, ends_at}`，优惠倍率必须小于 10000，例如 `5000` 表示在原有价格和分组基础倍率上再乘 `0.5`；请求开始时间命中 `[starts_at, ends_at)` 时才应用，并将最终倍率保存到用量快照。
...
提前断开的 OpenAI 兼容网关。上游返回重定向时网关不会自动跟随到新地址；重定向按非 `2xx`
失败处理，在当前渠道重试两次，三次请求均失败后切换已配置的下一候选渠道，提供商凭据不会发送到
重定向目标。
...
以保持多工具回传顺序合法。
网关按照提供商的 `weight` 从高到低请求，权重相同时保持稳定的路由顺序。DNS、拨号或 TLS 等发生在取得连接前的明确连接失败，会
在当前渠道有限重试并继续下一候选渠道。明确收到非 `2xx` 或重定向响应时，当前渠道重试两次；三次请求均失败后继续下一候选，全部候选耗尽才结束 operation 并释放预占。
已取得连接后发生网络错误、`2xx` 响应体读取失败、没有明确终态的流，或其他无法证明请求结果的情况不会重放，而是进入 `pending_unknown` 并保留预占供
...
兼容 Anthropic Messages 风格请求，并仅路由到支持 Anthropic 协议的提供商：
...
- 价格由管理员自行配置，不从厂商目录自动导入。模型价格不支持未来生效排期：固定价和分时价都在
  发布后立即生效，发布时间和替换时间只作为版本审计边界。分时价按 IANA 时区、星期和分钟窗口选择
  特殊费率，未命中窗口时使用该方案的默认费率。解析结果同时用于余额预占和最终结算，请求中途跨过
  峰谷边界不会改价。
  已发布方案按模型缓存在网关进程的共享内存快照中，同一模型不会为每个请求重复查询价格表；发布新版本
  会替换该模型快照，下一次请求立即按新版本解析。没有已发布方案的旧模型继续走兼容价格查询路径。
- 计费分组的基础倍率和定时优惠同样使用进程内共享快照。API Key 认证结果用于首次填充；通过管理接口
  新增、修改、清除优惠以及启停、删除分组后，会立即更新或移除对应缓存项。请求开始时确定的最终倍率
  会固定写入用量记录，活动边界之后的请求自动使用新倍率。
