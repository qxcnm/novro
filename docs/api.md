...
权重相同时保持稳定的路由顺序。建立响应前的连接错误和所有非 `2xx` 响应在当前提供商重试两次，
三次均失败后再尝试池中的下一条路由；收到 `2xx` 后，普通响应在完整读取前失败时切换下一条路由。
所有候选提供商均失败后返回 `upstream_unavailable` 并退还预占余额。普通响应只有在完整读取并完成结算后才写给客户端；SSE
