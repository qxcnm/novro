...
func fromEnt(entity *ent.ModelRoute) Record {
	...
	if p != nil {
		summary = ProviderSummary{ID: p.ID, Code: p.Code, DisplayName: p.DisplayName, Weight: p.Weight, Protocols: provider.ProtocolsFromStrings(p.Protocols, provider.Protocol(p.Protocol)), OutboundFormat: provider.OutboundFormat(p.OutboundFormat), Status: provider.Status(p.Status)}
	}
