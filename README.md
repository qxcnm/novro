...
- 支持按提供商权重选择模型路由、仅在确认尚未建立连接时安全重试与故障切换，以及按上游确认的 usage 结算。
- 提供桌面侧边栏、移动端导航抽屉、浅色和深色主题。

## 多提供商路由流程

同一个计费分组和对外模型可以配置多个提供商。权重越高的提供商越优先；
权重相同时保持稳定的路由顺序。模型调用是有副作用的 POST，只能在确认尚未取得
上游连接时有限重试或切换渠道。一旦取得连接、收到任意 HTTP 响应或请求结果无法确认，
系统不会自动重放：
