...
- 支持按提供商权重选择模型路由、对明确的非 `2xx` 响应重试两次后故障切换，以及按上游确认的 usage 结算。
- 提供桌面侧边栏、移动端导航抽屉、浅色和深色主题。

## 多提供商路由流程

同一个计费分组和对外模型可以配置多个提供商。权重越高的提供商越优先；
权重相同时保持稳定的路由顺序。明确收到非 `2xx` 响应时，当前渠道会重试两次，
三次请求均失败后切换下一渠道；请求可能已经送达但没有明确响应、响应体不完整或流已经开始时，
系统不会自动重放：
...
按照提供商 weight 从高到低排序
    ↓
请求权重最高的提供商
    ├─ 连接建立前明确失败：有限重试，再尝试下一提供商
    ├─ 收到非 2xx：当前提供商重试两次，三次均失败则尝试下一提供商
    ├─ 收到成功终态和明确 usage：按 usage 结算，多退少补
    └─ 已取得连接但结果不明：进入 pending_unknown
                                  ↓
                         保留预占并人工核对
```

预占不是最终消费。输入按 JSON 请求字节数估算，并默认最多预占 16384 Token；输出默认
最多预占 1024 Token。两个上限都可以在 `/admin/gateway` 调整，不限制实际上下文或输出。
最终只按上游明确报告的 usage 扣费，真实费用超过预占时通过唯一结算流水幂等补扣。
