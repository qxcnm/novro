...
type APIKeyRecord = { id: string; billing_group_id: string; billing_group: { id: string; code: string; display_name: string; kind?: "standard" | "composite"; member_group_ids?: string[]; multiplier_bps: number; effective_multiplier_bps?: number }; name: string; key_prefix: string; can_copy_secret: boolean; status: "active" | "revoked"; last_used_at: string | null; created_at: string; revoked_at: string | null };
type CreateResult = { api_key: APIKeyRecord; key: string };
type BillingGroup = { id: string; display_name: string; kind?: "standard" | "composite"; member_group_count?: number; multiplier_bps: number; effective_multiplier_bps?: number; is_default: boolean; status: "active" | "disabled" };
    ...
    <>
      <div className="space-y-5">
        ...
        <Dialog onOpenChange={(open) => { setCreateOpen(open); setError(""); if (!open) { setCreated(null); setCopied(false); setName(""); setBillingGroupID(""); } }} open={createOpen}>
          <DialogContent>{created ? <><DialogHeader><DialogTitle>保存你的 API Key</DialogTitle><DialogDescription>完整密钥只显示这一次。关闭后仍可在列表中重新复制。</DialogDescription></DialogHeader><div className="space-y-3"><Label htmlFor="created-api-key">API Key</Label><div className="flex gap-2"><Input className="font-mono text-xs" id="created-api-key" readOnly value={created.key} /><Button aria-label="复制 API Key" onClick={() => void copyKey()} size="icon" title="复制 API Key" variant="outline">{copied ? <Check /> : <Clipboard />}</Button></div><p className="text-xs text-muted-foreground">计费分组：{created.api_key.billing_group?.display_name ?? "默认分组"}{created.api_key.billing_group?.kind === "composite" ? " · 主分组，按命中成员结算" : ` · ${((created.api_key.billing_group?.effective_multiplier_bps ?? created.api_key.billing_group?.multiplier_bps ?? 10_000) / 10_000).toFixed(4)}×`}</p>{error ? <p className="text-sm text-destructive" role="alert">{error}</p> : null}</div><DialogFooter><Button onClick={() => setCreateOpen(false)}>{copied ? "已保存，关闭" : "我已保存"}</Button></DialogFooter></> : <><DialogHeader><DialogTitle>创建 API Key</DialogTitle><DialogDescription>为不同环境使用独立名称和计费分组，便于单独撤销和成本管理。</DialogDescription></DialogHeader><form className="space-y-4" id="create-key-form" onSubmit={createKey}><div className="space-y-2"><Label htmlFor="key-name">名称</Label><Input autoFocus id="key-name" maxLength={64} onChange={(event) => setName(event.target.value)} placeholder="例如 Production" required value={name} /></div><div className="space-y-2"><Label htmlFor="key-billing-group">计费分组</Label><Select onValueChange={setBillingGroupID} value={billingGroupID}><SelectTrigger id="key-billing-group"><SelectValue placeholder="选择计费分组" /></SelectTrigger><SelectContent>{(() => { const composites = billingGroups.filter((group) => group.kind === "composite"); const standards = billingGroups.filter((group) => group.kind !== "composite"); return <><SelectGroup>{composites.length > 0 ? <SelectLabel>主分组</SelectLabel> : null}{composites.map((group) => <SelectItem key={group.id} value={group.id}>{group.display_name} · 包含 {group.member_group_count ?? 0} 个分组 · 按命中模型结算</SelectItem>)}</SelectGroup><SelectGroup>{standards.length > 0 ? <SelectLabel>单一计费分组</SelectLabel> : null}{standards.map((group) => <SelectItem key={group.id} value={group.id}>{group.display_name} · {((group.effective_multiplier_bps ?? group.multiplier_bps) / 10_000).toFixed(4)}×</SelectItem>)}</SelectGroup></>; })()}</SelectContent></Select></div>{error ? <p className="text-sm text-destructive" role="alert">{error}</p> : null}</form><DialogFooter><Button onClick={() => setCreateOpen(false)} type="button" variant="outline">取消</Button><Button disabled={busy} form="create-key-form" type="submit"><KeyRound />{busy ? "正在创建..." : "创建"}</Button></DialogFooter></>}</DialogContent>
        </Dialog>
