        ...
        <DialogContent>
          ...
          <form className="space-y-4" id="create-provider-form" onSubmit={createProvider}>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2"><Label htmlFor="provider-code">提供商代码</Label><Input autoComplete="off" id="provider-code" maxLength={64} minLength={3} onChange={(event) => setCreateForm({ ...createForm, code: event.target.value.toLowerCase() })} pattern="[a-z0-9][a-z0-9.-]{1,62}[a-z0-9]" placeholder="例如 kimi-0.2" required title="提供商代码需为 3 到 64 位，只能使用小写字母、数字、点号和连字符，不能包含下划线或空格，且必须以字母或数字开头和结尾" value={createForm.code} /><p className="text-xs text-muted-foreground">3 到 64 位；允许小写字母、数字、点号和连字符，不允许下划线或空格。</p></div>
