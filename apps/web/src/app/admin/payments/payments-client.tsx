...
export default function PaymentsClient() {
  ...
  function saveMethod(event: FormEvent<HTMLFormElement>) {
    ...
    if (!/^[a-z0-9][a-z0-9_-]{0,31}$/.test(code) || !name || minMicros === null) { setError("支付方式名称、处理标识或最低充值金额无效"); return; }
  ...
  function addPreset() {
    ...
    if (micros === null || form.preset_amounts_micros.includes(micros)) { setError("请输入未添加过的有效充值金额"); return; }
  ...
  function saveBonus(event: FormEvent<HTMLFormElement>) {
    ...
    if (thresholdMicros === null || !Number.isFinite(percent) || percent <= 0 || percent > 100) { setError("赠送门槛和赠送比例无效"); return; }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (minMicros === null || maxMicros === null || minMicros > maxMicros) { setError("全局充值金额范围无效"); return; }
  ...
  return (
    <div className="space-y-5">
          ...
          <Tabs className="min-w-0 flex-1" defaultValue="rules">
            ...
            <TabsContent className="mt-5 space-y-5" value="rules">
              <div className="grid gap-5 lg:grid-cols-2">
                <Card><CardHeader><CardTitle>金额范围</CardTitle><CardDescription>用户提交的支付金额必须位于此范围内。</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="payment-min">最低充值</Label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">¥</span><Input className="pl-7" id="payment-min" inputMode="decimal" onChange={(event) => setForm({ ...form, min_amount: event.target.value })} value={form.min_amount} /></div></div><div className="space-y-2"><Label htmlFor="payment-max">最高充值</Label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">¥</span><Input className="pl-7" id="payment-max" inputMode="decimal" onChange={(event) => setForm({ ...form, max_amount: event.target.value })} value={form.max_amount} /></div></div></CardContent></Card>
                <Card><CardHeader><CardTitle>预设充值金额</CardTitle><CardDescription>这些金额会显示在用户充值弹窗中。</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex flex-wrap gap-2">{form.preset_amounts_micros.map((amount) => <Badge className="gap-1 py-1 pl-2.5" key={amount} variant="secondary">{formatMoney(amount)}<button aria-label={`删除预设金额 ${formatMoney(amount)}`} className="rounded-sm p-0.5 hover:bg-background" onClick={() => setForm((current) => ({ ...current, preset_amounts_micros: current.preset_amounts_micros.filter((item) => item !== amount) }))} type="button"><XCircle className="size-3.5" /></button></Badge>)}</div><div className="flex gap-2"><Input aria-label="新增预设充值金额" inputMode="decimal" onChange={(event) => setPresetInput(event.target.value)} placeholder="例如 200" value={presetInput} /><Button disabled={!presetInput.trim()} onClick={addPreset} type="button" variant="outline"><Plus />添加</Button></div></CardContent></Card>
              </div>
      ...
      <Dialog onOpenChange={setMethodOpen} open={methodOpen}><DialogContent><DialogHeader><DialogTitle>{methodIndex === null ? "添加支付方式" : "编辑支付方式"}</DialogTitle><DialogDescription>处理标识需与易支付平台支持的 type 值一致。</DialogDescription></DialogHeader><form className="space-y-4" id="payment-method-form" onSubmit={saveMethod}><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="method-name">显示名称</Label><Input id="method-name" maxLength={32} onChange={(event) => setMethodForm({ ...methodForm, name: event.target.value })} placeholder="例如 支付宝" required value={methodForm.name} /></div><div className="space-y-2"><Label htmlFor="method-code">处理标识</Label><Input id="method-code" maxLength={32} onChange={(event) => setMethodForm({ ...methodForm, code: event.target.value })} placeholder="例如 alipay" required value={methodForm.code} /></div><div className="space-y-2"><Label htmlFor="method-icon">图标</Label><Select onValueChange={(value) => setMethodForm({ ...methodForm, icon: value })} value={methodForm.icon}><SelectTrigger className="w-full" id="method-icon"><SelectValue /></SelectTrigger><SelectContent>{iconOptions.map((option) => <SelectItem key={option.value} value={option.value}><MethodIcon icon={option.value} />{option.label}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="method-min">最低充值</Label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">¥</span><Input className="pl-7" id="method-min" inputMode="decimal" onChange={(event) => setMethodForm({ ...methodForm, min_amount: event.target.value })} required value={methodForm.min_amount} /></div></div></div><label className="flex items-center gap-3 rounded-md border p-3"><Checkbox aria-label="启用支付方式" checked={methodForm.enabled} onCheckedChange={(checked) => setMethodForm({ ...methodForm, enabled: checked === true })} /><span className="text-sm font-medium">启用该支付方式</span></label></form><DialogFooter><Button onClick={() => setMethodOpen(false)} type="button" variant="outline">取消</Button><Button form="payment-method-form" type="submit"><Save />保存方式</Button></DialogFooter></DialogContent></Dialog>

      <Dialog onOpenChange={setBonusOpen} open={bonusOpen}><DialogContent><DialogHeader><DialogTitle>{bonusIndex === null ? "添加赠送档位" : "编辑赠送档位"}</DialogTitle><DialogDescription>用户支付金额不变，赠送部分直接计入到账余额。</DialogDescription></DialogHeader><form className="grid gap-4 sm:grid-cols-2" id="bonus-tier-form" onSubmit={saveBonus}><div className="space-y-2"><Label htmlFor="bonus-threshold">充值门槛</Label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">¥</span><Input className="pl-7" id="bonus-threshold" inputMode="decimal" onChange={(event) => setBonusForm({ ...bonusForm, threshold: event.target.value })} required value={bonusForm.threshold} /></div></div><div className="space-y-2"><Label htmlFor="bonus-percent">赠送比例</Label><div className="relative"><Input className="pr-8" id="bonus-percent" inputMode="decimal" max="100" min="0.01" onChange={(event) => setBonusForm({ ...bonusForm, percent: event.target.value })} required value={bonusForm.percent} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">%</span></div></div></form><DialogFooter><Button onClick={() => setBonusOpen(false)} type="button" variant="outline">取消</Button><Button form="bonus-tier-form" type="submit"><Save />保存档位</Button></DialogFooter></DialogContent></Dialog>
