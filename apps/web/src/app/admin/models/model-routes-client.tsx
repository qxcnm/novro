      ...
      <Dialog onOpenChange={(open) => { setEditorOpen(open); if (!open) { setEditing(null); setForm(EMPTY_FORM); } }} open={editorOpen}>
        <DialogContent>
          ...
          <form className="space-y-4" id="model-route-form" onSubmit={submit}>
            ...
            <div className="space-y-2"><Label htmlFor="route-public-name">对外模型名称</Label><Input disabled={editing !== null} id="route-public-name" maxLength={256} onChange={(event) => setForm({ ...form, public_name: event.target.value })} placeholder="例如 deepseek-chat 或 GPT-5.6 Luna" required title="对外模型名称不能为空，最多 256 个字符；首尾空白会被去除" value={form.public_name} /><p className="text-xs text-muted-foreground">允许上游原始模型名称（包括空格、中文和特殊字符），最多 256 个字符，首尾空白会被去除。</p></div>
